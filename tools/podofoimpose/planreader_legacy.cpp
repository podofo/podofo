    ...
    for (unsigned iter = 0; iter < maxIter; iter++)
    {
        ...
        for (int subi = numline + 1; subi < endOfloopBlock; subi++)
        {
            ...
            else
            {
                ...
                if (!p.isValid() || p.sourcePage > m_imp->m_sourceVars.PageCount)
                    continue;
    ...
    : m_imp(&imp)
{
    ...
    /// PROVIDED 
    m_imp->vars[string("$PagesCount")] = Util::uToStr(m_imp->m_sourceVars.PageCount);
    m_imp->vars[string("$SourceWidth")] = Util::dToStr(m_imp->m_sourceVars.PageWidth);
    m_imp->vars[string("$SourceHeight")] = Util::dToStr(m_imp->m_sourceVars.PageHeight);
    /// END OF PROVIDED
