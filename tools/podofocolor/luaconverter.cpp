...
LuaConverter::LuaConverter(const std::string_view& luaScript)
    : IConverter()
{
    if (imp_lua_dofile(m_machina.State(), luaScript.data()))
    {
...
void LuaConverter::StartPage(PdfPage&, int pageNumber)
{
    lua_getglobal(m_machina.State(), FUNCTION_START_PAGE);
    lua_pushnumber(m_machina.State(), static_cast<double>(pageNumber));
    // Lua 5.1 only
    //lua_pushinteger( m_machina.State(), nPageNumber );
    lua_call(m_machina.State(), 1, 0);
}

void LuaConverter::EndPage(PdfPage&, int pageNumber)
{
...
PdfColor LuaConverter::GetColorFromReturnValue(const char* functionName)
{
    int top;
    double value;
    ...
    switch (len)
    {
        ...
        default:
        {
            PoDoFo::LogMessage(PdfLogSeverity::Error, "Array length is %i returned by %s.\n", len, functionName);
            PODOFO_RAISE_ERROR_INFO(PdfErrorCode::CannotConvertColor,
...
PdfColor LuaConverter::SetStrokingColorGray(const PdfColor& color)
{
    lua_getglobal(m_machina.State(), FUNCTION_SET_STROKING_GRAY);
    lua_pushnumber(m_machina.State(), color.GetGrayScale());
    lua_call(m_machina.State(), 1, 1);

    return this->GetColorFromReturnValue(FUNCTION_SET_STROKING_GRAY);
}

PdfColor LuaConverter::SetStrokingColorRGB(const PdfColor& color)
{
    lua_getglobal(m_machina.State(), FUNCTION_SET_STROKING_RGB);
    lua_pushnumber(m_machina.State(), color.GetRed());
    lua_pushnumber(m_machina.State(), color.GetGreen());
    lua_pushnumber(m_machina.State(), color.GetBlue());
    lua_call(m_machina.State(), 3, 1);

    return this->GetColorFromReturnValue(FUNCTION_SET_STROKING_RGB);
}

PdfColor LuaConverter::SetStrokingColorCMYK(const PdfColor& color)
{
    lua_getglobal(m_machina.State(), FUNCTION_SET_STROKING_CMYK);
    lua_pushnumber(m_machina.State(), color.GetCyan());
    lua_pushnumber(m_machina.State(), color.GetMagenta());
    lua_pushnumber(m_machina.State(), color.GetYellow());
    lua_pushnumber(m_machina.State(), color.GetBlack());
    lua_call(m_machina.State(), 4, 1);

    return this->GetColorFromReturnValue(FUNCTION_SET_STROKING_CMYK);
}

PdfColor LuaConverter::SetNonStrokingColorGray(const PdfColor& color)
{
...
PdfColor LuaConverter::SetNonStrokingColorRGB(const PdfColor& color)
{
...
PdfColor LuaConverter::SetNonStrokingColorCMYK(const PdfColor& color)
{
