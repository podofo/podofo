...
void PdfNameTreeNode::SetLimits() 
{
    ...
    if( m_bHasKids )
    {
        PdfObject* pChild = m_pObject->GetParent()->GetObject( (*m_pObject->GetDictionary().GetKey("Kids")->GetArray().begin()).GetReference() );
        limits.push_back( *(pChild->GetDictionary().GetKey("Limits")->GetArray().begin()) );

        pChild = m_pObject->GetParent()->GetObject( m_pObject->GetDictionary().GetKey("Kids")->GetArray().back().GetReference() );
...
bool PdfNameTreeNode::Rebalance()
{
    ...
    if( pArray->size() > nLength )
    {
        ...
        PdfObject* pChild1;
        PdfObject* pChild2 = m_pObject->GetParent()->CreateObject();

        if( !m_pParent ) 
        {
            m_bHasKids = true;
            pChild1    = m_pObject->GetParent()->CreateObject();
...
PdfObject* PdfNamesTree::GetKeyValue( PdfObject* pObj, const PdfString & key ) const
{
    ...
    if( pObj->GetDictionary().HasKey("Kids") )
    {
        ...
        while( it != kids.end() )
        {
            PdfObject* pChild = m_pObject->GetParent()->GetObject( (*it).GetReference() );
            if( pChild ) 
