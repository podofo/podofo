...
int PdfRijndael::init(Mode mode,Direction dir,const pdf_uint8 * key,KeyLength keyLen,pdf_uint8 * initVector)
{
  // Not initialized yet
  m_state = Invalid;
  ...
  pdf_uint8 keyMatrix[_MAX_KEY_COLUMNS][4];

  for(pdf_uint32 i = 0;i < uKeyLenInBytes;i++)keyMatrix[i >> 2][i & 3] = key[i]; 
...
int PdfRijndael::blockEncrypt(const pdf_uint8 *input,int inputLen,pdf_uint8 *outBuffer)
{
  int i, k, numBlocks;
  pdf_uint8 block[16], iv[4][4];
...
pdf_long PdfRijndael::padEncrypt(const pdf_uint8 *input, pdf_long inputOctets, pdf_uint8 *outBuffer)
{
  pdf_long i, numBlocks, padLen;
  pdf_uint8 block[16], *iv;
...
int PdfRijndael::blockDecrypt(const pdf_uint8 *input, int inputLen, pdf_uint8 *outBuffer)
{
  int i, k, numBlocks;
  pdf_uint8 block[16], iv[4][4];
...
int PdfRijndael::padDecrypt(const pdf_uint8 *input, int inputOctets, pdf_uint8 *outBuffer)
{
...
void PdfRijndael::keySched(pdf_uint8 key[_MAX_KEY_COLUMNS][4])
{
  int j,rconpointer = 0;
  ...
  pdf_uint8 tempKey[_MAX_KEY_COLUMNS][4];
...
void PdfRijndael::encrypt(const pdf_uint8 a[16], pdf_uint8 b[16])
{
  pdf_uint32 r;
  pdf_uint8 temp[4][4];
