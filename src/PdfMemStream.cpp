...
void PdfMemStream::GetFilteredCopy( char** ppBuffer, long* lLen ) const
{
...
void PdfMemStream::FreeDecodeParms( TVecDictionaries* pParams ) const
{
...
void PdfMemStream::FillFilterList( TVecFilters & vecFilters ) const
{
    PdfObject* pObj;
    ...
    if( m_pParent->GetDictionary().HasKey( "Filter" ) )
    {
        ...
        else if( pObj->IsArray() )
        {
            ...
            while( it != vecVariants.end() )
            {
                ...
                eFilter = PdfMemStream::FilterNameToType( (*it).GetName() );
                vecFilters.push_back( eFilter );
...
EPdfFilter PdfMemStream::FilterNameToType( const PdfName & name )
{
...
void PdfMemStream::GetDecodeParms( TVecDictionaries* pParams ) const
{
    PdfObject*               pObj = NULL;
    PdfArray                 array;
...
void PdfMemStream::SetDecodeParms( TVecDictionaries* pParams )
{
    PdfArray             array;    
